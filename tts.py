...
def text_to_vedio(input, output):
    SSML_text = get_SSML(input)
    asyncio.get_event_loop().run_until_complete(mainSeq(SSML_text, output))

def get_xml(text, voice_name="zh-CN-XiaoxiaoNeural", rate="0%", pitch="0%"):
    with open(main_dir + r"\SSML.xml", "w", encoding='utf-8') as f:
        f.write('<speak xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="http://www.w3.org/2001/mstts" xmlns:emo="http://www.w3.org/2009/10/emotionml" version="1.0" xml:lang="en-US">')
        f.write('\n')
        f.write(r'    <voice name="' + voice_name + r'">')
        f.write('\n')
        f.write(r'        <prosody rate="' + rate + r'" pitch="' + pitch + r'">')
        f.write('\n')
        f.write('        ')
        f.write(text)
        f.write('\n')
